...
module.exports = {
    run: function(creep) {
        ...
        let delTp = creep.memory.delTp;
        let getTp = creep.memory.getTp;
        
        let delT = creep.memory.delT;
        let getT = creep.memory.getT;
        
        if ((delT==undefined&&getT==undefined&&creep.ticksToLive<800)||creep.memory.recycle==true) {
            // go back home and recycle
        }
        else { // no recycle do job
            let jobState = creep.memory.jobState||'d';
            if (jobState=='d') {
                if (delTp) {
                    // if not full
                        // if store has
